...
public class ResultAnalyzer {
    ...
    private final FileProcess[] procesos;

    public ResultAnalyzer(FileProcess[] procesos) {
        ...
        try {
            ...
            try (PrintWriter pw = new PrintWriter(new FileWriter(rutaArchivo, true))) {
                ...
                for (int i = 0; i < procesos.length; i++) {
                    pw.println("Thread " + (i + 1)
                            + "," + procesos[i].getTiempoObtenerLogger()
                            + "," + procesos[i].getTiempoEscritura());
                }
